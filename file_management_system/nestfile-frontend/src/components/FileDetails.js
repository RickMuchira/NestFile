...
const FileDetails = () => {
  ...
  const handleRename = () => {
    if (!newFileName.trim()) {
      alert("File name cannot be empty or only whitespace!");
      return;
    }
    ...
    const data = {
      ...
      directory: fileDetails.directory, // Ensure the directory ID is correctly set
    };

    axios.put(`/api/files/${fileId}/`, data, { headers: { 'Content-Type': 'application/json' } })
      .then(response => {
      ...
      .catch(error => {
        ...
        alert(`Failed to rename file. Server responded with: ${error.message}`);
      });
  };
  ...
  return (
    <div className="file-details bg-white p-6 rounded shadow-md">
      ...
      <p className="text-gray-600">Directory ID: {fileDetails.directory}</p>
